(function () {
  ...
  obj.getInfo = function (category) {
    if (category == "name") {
      return "Get Current Balance of ETH";
    } else if (category == "description") {
      return "Get the current balance of ETH for a specific wallet address and store it in variables.";
      ...
      return [{
        id: 1,
        name: "Get Current Balance of ETH",
        description: "Get the current balance of ETH and store it in variables.",
        parameter: [
          {
            id: 1,
            name: "ObjectID:",
  ...
  obj.execActionCommand = function (index, valueJson, objectId, instanceId, actionId, commandId) {
    ...
    switch (paramId) {
      case 1:
        ...
        var balanceVar = obj.getVariable(objId, obj.getValueJson(valueJson, 201), instanceId);
        var walletAddress = obj.getValueJson(valueJson, 202);
        var objectVariable = obj.getVariable(objId, obj.getValueJson(valueJson, 203), instanceId);

        // Define a function to get the current balance of ETH from the API
        function getEthBalance() {
          var xhr = new XMLHttpRequest();
          xhr.open("GET", "https://my-proxy-nine.vercel.app/" + encodeURIComponent(walletAddress));
          xhr.onload = function () {
            if (xhr.status === 200) {
              var data = JSON.parse(xhr.responseText);
              var ethBalance = data.ethBalance;
              balanceVar.setValue(ethBalance);
              if (objectVariable) {
                objectVariable.setValue(ethBalance);
              }
            } else {
        ...
        // Call the function to get the current balance of ETH
        getEthBalance();

        // Set an interval to call the function every 5 seconds
        setInterval(getEthBalance, 5000);
